...
SCRIPTS=/projects/tir1/users/xinyiw1/usr/local/mosesdecoder/scripts
...
UDIR=$BASE/utils
vocab_size=16000
cat $prep/train.de-en.de $prep/train.de-en.en > $prep/combine.de-en
#python $UDIR/train-spm.py \
#  --input="$prep"/combine.de-en \
#  --model_prefix="$prep"/spm"$vocab_size" \
...
for l in $src $tgt; do
  ...
  do
    python $UDIR/run-spm.py \
      --model="$prep"/spm"$vocab_size".model \
      < $f \
      > ${f/de-en.$l/de-en.spm$vocab_size.$l} 
  done
