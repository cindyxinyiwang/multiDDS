...
MODEL_DIR=checkpoints/score_debug/debug/
mkdir -p $MODEL_DIR
...
CUDA_VISIBLE_DEVICES=$1 python train.py data-bin/score_debug/ \
	  --task translation \
    ...
    --share-all-embeddings \
	  ...
	  --seed 2 \
  	  ...
  	  --save-dir $MODEL_DIR \
          ...
          --scale-norm \
    --data-actor "transformer" \
    --data-actor-step-update \
    --data-actor-lr-scheduler 'inverse_sqrt_decay' \
	  --data-actor-feature-postprocess 'tanh' \
	  --out-score-type 'none' \
    --score-log $MODEL_DIR/stats \
