...
#exturt PYTHONPATH="$(pwd)"
#exturt CUDA_VISIBLE_DEVICES="2"

MODEL_DIR=checkpoints/bt_azetur/aze_up1_dual_v2_s2/
mkdir -p $MODEL_DIR
...
CUDA_VISIBLE_DEVICES=$1 python train.py data-bin/bt_azetur/ \
	  --task bt_translation \
    ...
    --share-all-langpair-embeddings \
	  ...
	  --reset-bt-dropout --bt-dropout 0.3 --bt-attention-dropout 0.3 --bt-relu-dropout 0.3 --weight-decay 0.0 \
	  --no-epoch-checkpoints \
    --bt-norm-by-word \
    --bt-optimizer-momentum 0.9 --bt-optimizer-nesterov \
    --dual --lm-path checkpoints/bt_azetur/lm/checkpoint_best.pt \
