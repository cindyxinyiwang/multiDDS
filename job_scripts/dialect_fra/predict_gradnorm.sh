...
MODEL_DIR=checkpoints/dialect_fra/prediction_v1/
mkdir -p $MODEL_DIR
...
CUDA_VISIBLE_DEVICES=$1 python get_grad_norm.py data-bin/dialect_fra_predict/ \
	  ...
	  --seed 2 \
  	  --save-dir $MODEL_DIR \
	  --log-interval 100 >> $MODEL_DIR/gradnorm.log 2>&1
          #--no-epoch-checkpoints \
  #--utility-type 'ave' \
  #--data-actor 'ave_emb' \
  #--data-actor-multilin \
