...
#excest PYTHONPATH="$(pwd)"
#excest CUDA_VISIBLE_DEVICES="2"

MODEL_DIR=checkpoints/bt_slkces/slk_up0_dds_s10t1.2_m_dbase_lr1e-5_dis0.99_v1/
mkdir -p $MODEL_DIR
...
CUDA_VISIBLE_DEVICES=$1 python train.py data-bin/bt_slkces/ \
	  ...
	  --max-epoch 50 \
          ...
          --lang-pairs "slk-eng" \
          --eval-lang-pairs "slk-eng" \
	  --reset-optimizer --reset-meters --reset-dataloader \
    --only-optim-model-key "slk-eng" \
    ...
    --reward-scale 1 \
    --data-actor-lr 1e-5 \
    --sampling --sampling-topk 10 --temperature 1.2 \
