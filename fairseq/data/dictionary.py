...
class CharNgramDictionary(object):
    """A mapping from symbols to consecutive integers"""
    ...
    def finalize(self, threshold=-1, nwords=8000, padding_factor=8):
        """Sort symbols by frequency in descending order, ignoring special ones.
    ...
    def _get_char_kv(self, word):
        kv = {}
        vsize = len(self.symbols)
        for i in range(len(word)):
            for j in range(i+1, min(len(word), i+self.char_n)+1):
                ngram = word[i:j] 
                if ngram in self.indices:
                    ngram = self.indices[ngram]
                else:
                    ngram = self.unk()
                if ngram not in kv: kv[ngram] = 0
                kv[ngram] += 1
        #key = torch.LongTensor([[0 for _ in range(len(kv.keys()))], list(kv.keys())])
        #val = torch.FloatTensor(list(kv.values()))
        #emb = torch.sparse.FloatTensor(key, val, torch.Size([1, vsize]))
        #return emb
        return kv
    ...
    def encode_line(self, line, line_tokenizer=tokenize_line, add_if_not_exist=True,
        ...
        for i, word in enumerate(words):
            ...
            #else:
            #    idx = self.index(word)
            char_kv = self._get_char_kv(word)
            if consumer is not None:
                consumer(word, char_idx_tensor)
            ids.append(char_kv)
        return ids
    ...
    def add_file_to_dictionary(filename, dict, tokenize, num_workers):
        ...
        if num_workers > 1:
            ...
            for worker_id in range(num_workers):
                results.append(pool.apply_async(
                    CharNgramDictionary._add_file_to_dictionary_single_worker,
                    (filename, tokenize, dict.eos_word, worker_id, num_workers)
                ))
            pool.close()
            pool.join()
            ...
            merge_result(CharNgramDictionary._add_file_to_dictionary_single_worker(filename, tokenize, dict.eos_word))
